...
def format_block(lines, infostr=""):
    ...
    if (not "/**" in lines[0]
          or not "*/" in lines[-1]):
        print("%s not a code block"%infostr)
        return lines
    ...
    if not lines[0].strip().startswith("/**"):
        print ("%s error, ignoring code block with junk in same line before"%infostr)
        return lines
    if not lines[-1].strip().endswith("*/"):
        print ("%s error, ignoring code block not ending at end of line"%infostr)
        return lines

    if lines[0].strip()!="/**":
        #print ("%s warning code block not starting in separate line"%infostr)
        idx = lines[0].find("/**")
        temp = [lines[0][0:idx+3], lines[0][idx+3:]]
        temp.extend(lines[1:])
        lines = temp
    if lines[-1].strip()!="*/":
        #print ("%s warning code block not ending in separate line"%infostr)
        idx = lines[-1].find("*/")
        temp = lines[0:-1]
        ...
        if one_in(ops_separate_line, lines[idx]):
            ...
            for it in ops_separate_line:
                if it in thisline and thisline!=it:
                    print ("%s warning %s not in separate line"%(infostr, it))
            ...
            if not thisline.split(" ")[0] in ops_title_line:
                print ("%s warning: @page/@name not at start of line"%(infostr))
            ...
            if not thisline.startswith("@ref") and not thisline.startswith("(@ref"):
                print ("%s warning %s not at start of line"%(infostr, "@ref"))
            ...
            if len(withquotes)==3:
                ...
                if len(withquotes[0].strip().split(' '))!=2:
                    print ("%s warning @ref line looks broken"%(infostr))     
                ...
                print ("%s warning @ref quotes are not in single line"%(infostr))
                remain = ''
            ...
            if not starts_with_one(ops_startline, thisline):
                for it in ops_startline:
                    if it in thisline:
                        print ("%s warning %s not at start of line"%(infostr, it))
            ...
            if "@f{" in lines[idx]:
                if not lines[idx].endswith("}{"):
                    print ("%s warning malformed @f{*}{"%(infostr))
            if curlines!=[]:
                out.extend(wrap_block(remove_junk(curlines), start))
                curlines=[]
            while True:
                ...
                if thisline.strip()=="*":
                    ...
                    if thisline[0:len(start)].strip()=="":
                        ...
                        print ("%s Error: wrong formatting inside @code block"%infostr)
                        thisline = start + thisline.strip()
                else:
...
print("%s where am I:"%args[0], file=sys.stderr)
