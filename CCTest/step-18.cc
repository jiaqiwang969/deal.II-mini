...
//
// 接下来，我们定义弹性中的应力和应变的线性关系。它由一个等级为4的张量给出，通常被写成  $C_{ijkl} = \mu (\delta_{ik} \delta_{jl} + \delta_{il}
 \delta_{jk}) + \lambda \delta_{ij} \delta_{kl}$  的形式。这个张量将等级2的对称张量映射到等级2的对称张量。对于Lam&eacute;常数 $\lambda$ 和 $\mu$ 的给定值，一个实现其创建的函数是直接的。
//
  template <int dim> 
