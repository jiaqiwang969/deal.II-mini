...
namespace Euler_DG
{
  ...
  // 这个参数指定共享内存组的大小。目前，只有1和 numbers::invalid_unsigned_int
  // 的值是可能的，这导致了内存功能可以被关闭，或者所有访问同一共享内存域的进程被分组。

  constexpr unsigned int group_size = numbers::invalid_unsigned_int;

  using Number = double;

  // 这里，数据结构的类型被选择为矢量化。在默认情况下，使用VectorizedArray<Number>，也就是说，在给定的硬件上使用最高的指令集架构扩展，具有最大数量的向量通道。然而，人们可能会减少填充通道的数量，例如，通过编写
  // <code>using VectorizedArrayType = VectorizedArray<Number, 4></code>
  // ，只处理4个单元。
  ...
  // 与Runge-Kutta有关的函数从 step-67
  // 复制过来，并稍作修改，以尽量减少全局矢量访问。
  ...
  private:
    // 包含子通信器的SubCommunicatorWrapper实例，我们需要将其传递给
    // MatrixFree::reinit() ，以便能够利用MPI-3.0的共享内存功能。
  ...
  // 新的构造函数，可以创建一个子通信器。用户可以通过全局参数group_size指定子通信器的大小。如果大小被设置为-1，一个共享内存域的所有MPI进程将被合并为一个组。指定的大小对于MatrixFree的共享内存能力的好处是决定性的，因此，设置为
  // <code>size</code> to <code>-1</code> 是一个合理的选择。通过设置
  // <code>1</code>
  // ，用户明确地禁用了MatrixFree的MPI-3.0共享内存功能，而完全依赖MPI-2.0功能，如
  // <code>MPI_Isend</code> 和 <code>MPI_Irecv</code>  。
  ...
  // 修改了 reinit() 函数，以设置 MatrixFree
  // 中的内部数据结构，使其可以被以单元为中心的循环使用，并使用 MPI-3.0
  // 的共享内存功能。
    ...
    const DoFHandler<dim> &dof_handler)
  {
    ...
    // 对单元格进行分类，使所有车道的每个面都有相同的边界ID。这在严格意义上是没有必要的，但是，可以在
    // EulerOperator::perform_stage()
    // 中写出更简单的代码，不需要屏蔽，因为可以保证所有分组的单元格（在一个VectorizedArray中）也要对面进行完全相同的操作。
  ...
  与 step - 67 相比，本教程的核心内容是 // --。
    ...
    // 与 step-67 相比，我们没有依次执行平流步骤（使用 MatrixFree::loop())
    // 和反质量矩阵步骤（使用 MatrixFree::cell_loop()) ），而是在
    // MatrixFree::loop_cell_centric(). 中一次性评估所有内容。
    // 这个函数期望在每个本地拥有的（宏）单元上执行一个单独的函数作为参数，这样我们就需要在该单元的所有面上循环，自行执行需要的积分步骤。

    // 以下函数在很大程度上包含了 step-67
    // 中的以下函数的副本，所以这里跳过了与弱形式的评估有关的评论。
      ...
      LinearAlgebra::distributed::Vector<Number> &      solution) const
  {
    ...
    // 通过调用 MatrixFree::loop_cell_centric()
    // 并提供一个包含单元、面和边界-面积分效果的lambda来运行以单元为中心的循环。
      ...
      [&](const auto &data, auto &dst, const auto &src, const auto cell_range) {
        ...
        for (unsigned int cell = cell_range.first; cell < cell_range.second;
             ++cell)
          {
            ...
            // 缓冲正交点的计算值，因为这些值在下一步被
            // FEEvaluation::submit_value() 所覆盖，但是，在后面的面积分中需要。

            for (unsigned int i = 0; i < phi.static_n_q_points * (dim + 2); ++i)
              buffer[i] = phi.begin_values()[i];

            // 在单元格正交点上应用单元格积分。也可参见来自  step-67  的函数
            // <code>EulerOperator::local_apply_cell()</code>  。
