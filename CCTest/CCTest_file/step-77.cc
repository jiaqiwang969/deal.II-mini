...
// 这个程序开始时和其他大多数程序一样，有众所周知的包含文件。与 step-15
// 程序相比，我们在这里所做的大部分工作都是从该程序中复制的，唯一不同的是包括头文件，我们从该文件中导入了SparseDirectUMFPACK类和KINSOL的实际接口。
...
namespace Step77
{
  ...
  // 同样地，这个程序的主类基本上是  step-15
  // 中的一个副本。然而，该类确实将雅各布（系统）矩阵（以及使用直接求解器对其进行因式分解）和残差的计算分成了不同的函数，原因已在介绍中列出。出于同样的原因，该类也有一个指向雅各布矩阵因式分解的指针，该指针在我们每次更新雅各布矩阵时被重置。
  ...
  // 最后，该类有一个定时器变量，我们将用它来评估程序的不同部分需要多长时间，这样我们就可以评估KINSOL的不重建矩阵及其因式分解的倾向是否合理。我们将在下面的
  // "结果 "部分讨论这个问题。
  ...
  // 然后，下面的函数负责对雅各布矩阵进行组装和因子化。该函数的前半部分实质上是
  // step-15
  // 的`assemble_system()`函数，只是它没有处理同时形成右手边的向量（即残差），因为我们并不总是要同时做这些操作。

  // 我们把整个装配功能放在一个由大括号包围的代码块中，这样我们就可以用一个
  // TimerOutput::Scope
  // 变量来衡量在这个代码块中花费了多少时间，不包括在这个函数中发生在匹配的闭合括号`}`之后的一切。
  ...
  // `assemble_system()`在 step-15
  // 中用来做的第二部分是计算残差向量，也就是牛顿线性系统的右手向量。我们把这一点从前面的函数中分解出来，但如果你理解了
  // step-15
  // 中`assemble_system()`的作用，下面的函数就会很容易理解。然而，重要的是，我们需要计算的残差不是围绕当前解向量线性化的，而是我们从KINSOL得到的任何东西。这对于诸如直线搜索这样的操作是必要的，我们想知道在不同的
  // $\alpha_k$ 值下，残差 $F(U^k + \alpha_k \delta U^K)$
  // 是多少；在这些情况下，KINSOL只是给我们函数 $F$
  // 的参数，然后我们在这时计算残差 $F(\cdot)$ 。
  ...
  // 接下来是实现用雅各布矩阵解线性系统的函数。由于我们在建立矩阵时已经对矩阵进行了因式分解，所以解决线性系统的方法就是将逆矩阵应用于给定的右侧向量。这就是我们在这里使用的
  // SparseDirectUMFPACK::vmult()
  // 函数的作用。在这之后，我们必须确保我们也能解决解向量中的悬空节点的值，而这是用
  // AffineConstraints::distribute(). 来完成的。

  // 该函数需要一个额外的，但未使用的参数`tolerance`，它表示我们必须解决线性系统的精确程度。这个参数的含义在介绍中结合
  // "Eisenstat Walker技巧
  // "进行了讨论，但由于我们使用的是直接求解器而不是迭代求解器，所以我们并没有利用这个机会只求解线性系统的不精确性。
  ...
  //  @sect4{Refining the mesh, setting boundary values, and generating
  //  graphical output}
    ...
    for (unsigned int refinement_cycle = 0; refinement_cycle < 6;
         ++refinement_cycle)
      {
...

        {
          ...
          // 所有这四种操作都由 SUNDIALS::KINSOL
          // 类的成员变量表示，这些成员变量的类型是 `std::function`,
          // ，即它们是我们可以分配给一个函数的指针的对象，或者像我们在这里做的那样，一个
          // "lambda函数"，它接受相应的参数并返回相应的信息。按照惯例，KINSOL希望做一些不重要的事情的函数返回一个整数，其中0表示成功。事实证明，我们只需用25行代码就可以完成所有这些工作。

          // 如果你不知道什么是 "lambda函数"，可以看看 step-12
          // 或[wikipedia页面](https:en.wikipedia.org/wiki/Anonymous_function)关于这个问题。lambda函数的想法是，人们想用一组参数来定义一个函数，但(i)不使它成为一个命名的函数，因为通常情况下，该函数只在一个地方使用，似乎没有必要给它一个全局名称；(ii)该函数可以访问存在于定义它的地方的一些变量，包括成员变量。lambda函数的语法很笨拙，但最终还是很有用的）。)
