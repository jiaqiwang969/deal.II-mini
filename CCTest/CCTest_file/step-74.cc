...
namespace Step74
{
  ...
  // @sect3{Equation data}  这里我们定义了两个测试案例：平滑函数的收敛率和
  // Functions::LSingularityFunction. 的l_singularity。
  enum class TestCase
  {
    convergence_rate,
    l_singularity
  ...
  //  @sect3{Auxiliary functions}  以下两个辅助函数分别用于计算一个面上的 $u_h$
  //  和 $\nabla u_h$ 的跳转项。
  ...
  // @sect3{The CopyData}  下面，我们为 MeshWorker::mesh_loop(), 定义 "复制
  // "对象，它与 step-12 本质上是一样的。注意，这里没有定义 "Scratch
  // "对象，因为我们用 MeshWorker::ScratchData<dim> 代替。复制 "和 "抓取
  // "对象的使用在WorkStream命名空间文档中得到了广泛的解释）。
  ...
  //  在这些准备工作之后，我们继续进行本程序的主类，称为`SIPGLaplace`。该类的整体结构与其他许多教程程序一样。主要的区别只出现在组装函数的实现上，因为我们使用FEInterfaceValues来组装面条。
  ...
  //  @sect3{The assemble_system function}  这里的组装功能与  step-12  和
  //  step-47
  //  中类似。与手工装配不同的是，我们只需要专注于在每个单元格、每个边界面和每个内部面进行装配。单元和面的循环由
  //  MeshWorker::mesh_loop().  自动处理。
  ...
  SIPGLaplace<dim>::assemble_system()
  {
    ...
    // 下面的lambda函数就会把数据复制到全局矩阵和右侧。
    // 虽然DG离散化中没有悬空节点约束，但我们定义了一个空的AffineConstraints对象，允许我们使用
    // AffineConstraints::distribute_local_to_global() 功能。
    ...
    // 在定义了装配函数后，我们现在可以创建ScratchData和CopyData对象，并将它们和上面的lambda函数一起传递给
    // MeshWorker::mesh_loop(). 此外，我们还需要指定我们要在内部面完全装配一次。
  ...
  // 这里的误差估计器的装配与全局矩阵和右前侧的装配很相似，可以由
  // MeshWorker::mesh_loop()
  // 框架处理。为了理解每个局部（lambda）函数的作用，首先回顾一下，局部单元残差的定义为
  ...
  SIPGLaplace<dim>::compute_error_estimate()
  {
    ...
    // 接下来计算边界条款  $\sum_{f\in \partial K \cap \partial \Omega} \sigma
    // \left\| [  u_h-g_D ]  \right\|_f^2  $  。
    ...
    // 在所有这些设置之后，让我们来做实际的工作。我们调整向量的大小，将结果写入其中，然后用
    // MeshWorker::mesh_loop() 函数驱动整个过程。
  ...
  // 接下来，我们用能量准则来评估准确性。这个函数类似于上面的误差估计器的集合。这里我们计算由
  // @f[
  ...
  template <int dim>
  double
  SIPGLaplace<dim>::compute_energy_norm_error()
  {
  ...
  //  @sect3{The compute_errors() function}  我们分别计算 $L_2$ 规范、 $H_1$
  //  半规范和能量规范中的三个误差。这些误差会被打印到屏幕上，同时也会存储在一个表格中，该表格记录了这些误差是如何随着网格细化而衰减的，并且可以在程序结束时一步步输出。
        ...
        switch (test_case)
          {
              ...
              output_results(cycle);
              {
                ...
                convergence_table.add_value("cells",
  ...
  //  @sect3{The main() function}  下面的 <code>main</code>
  //  函数与前面的例子也类似，不需要注释。
