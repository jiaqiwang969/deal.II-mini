...
// 包含文件的集合是相当标准的。最耐人寻味的部分是，我们将完全依靠deal.II数据结构进行MPI并行化，特别是通过
// parallel::distributed::Triangulation 和 LinearAlgebra::distributed::Vector
// 包含的 <code>distributed/tria.h</code> 和
// <code>lac/la_parallel_vector.h</code>  。我们将使用非分布式的
// dealii::SparseMatrix  (  <code>lac/sparse_matrix.h</code>  ) 来存储
// $\mathbf{c}_{ij}$  、  $\mathbf{n}_{ij}$  和  $d_{ij}$ 矩阵的本地部分，而不是
// Trilinos 或 PETSc 特定的矩阵类。
...
// 我们开始实际的实现，先声明所有的类及其数据结构和方法。与之前的例子步骤相比，我们使用了更精细的概念、数据结构和参数封装到各个类中。因此，一个单一的类通常围绕着一个单一的数据结构（如
// <code>Discretization</code> 类中的Triangulation），或者一个单一的方法（如
// <code>%TimeStepping</code> 类的 <code>make_one_step()</code>
// 函数）。我们通常声明参数变量和从头开始的数据对象为`private'，而使其他类使用的方法和数据结构为`public'。

//  @note  一个更简洁的方法是通过<a
//  href="https:en.wikipedia.org/wiki/Mutator_method">getter/setter
//  functions</a>来保护对所有数据结构的访问。为了简洁起见，我们不采用这种方法。

// 我们还注意到，绝大多数的类都是从ParameterAcceptor派生的。这有利于将所有的全局参数归入一个（全局）ParameterHandler。关于从ParameterAcceptor继承作为全局订阅机制的更多解释可以在
// step-60  中找到。
...
namespace Step69
{
  ...
  // 我们首先定义一些 types::boundary_id
  // 常量，用于整个教程步骤。这使得我们可以用一个助记符（如
  // <code>do_nothing</code>  ）而不是一个数值来指代边界类型。
  ...
  //  <code>Discretization</code>
  //  类包含所有关于问题的网格（三角形）和离散化（映射、有限元、正交）的数据结构。如前所述，我们使用ParameterAcceptor类来自动填充特定问题的参数，如几何信息（
  //  <code>length</code>  等）或来自参数文件的细化水平（
  //  <code>refinement</code>
  //  ）。这就要求我们把数据结构的初始化分成两个函数。我们在构造函数中初始化所有不依赖参数的东西，并将网格的创建推迟到
  //  <code>setup()</code> 方法中，一旦所有参数通过
  //  ParameterAcceptor::initialize(). 读入，就可以调用该方法。
  template <int dim>
  ...
  //  <code>OfflineData</code>
  //  类包含了离散化中几乎所有不随时间演变的组件，特别是DoFHandler、SparsityPattern、边界图、块状质量矩阵、
  //  $\mathbf{c}_{ij}$ 和 $\mathbf{n}_{ij}$
  //  矩阵。这里，术语<i>offline</i>指的是 <code>OfflineData</code>
  //  的所有类成员都有明确定义的值，与当前时间步长无关。这意味着它们可以提前初始化（在<i>time
  //  step
  //  zero</i>），并且不意味着在任何后来的时间步长中被修改。例如，稀疏模式不应该随着时间的推进而改变（我们在空间上不做任何形式的适应性）。同样地，包络质量矩阵的条目也不应该随着时间的推进而被修改。

  // 我们还计算并存储一个 <code>boundary_normal_map</code>
  // ，它包含一个从边界自由度的 types::global_dof_index
  // 类型的全局索引到一个由法向量、边界ID和与自由度相关的位置组成的元组的映射。我们必须在这个类中计算和存储这些几何信息，因为我们在后面的代数循环中无法获得几何（或基于单元）的信息。

  // 尽管这个类目前没有任何可以从参数文件中读入的参数，但我们还是从ParameterAcceptor派生出来，并遵循与Discretization类相同的习惯，提供一个
  // <code>setup()</code> (and <code>assemble()</code>  )方法。
  ...
  // - 类型别名  <code>flux_type</code>  用来表示通量
  // $\mathbb{f}(\mathbf{U}_j^n)$  。

  // -  <code>momentum</code> 函数从状态向量 $[\rho,\textbf{m},E]$ 中提取
  // $\textbf{m}$ 并存储在一个 <code>Tensor<1, dim></code> 中。

  // -  <code>internal_energy</code> 函数从给定的状态向量 $[\rho,\textbf{m},E]$
  // 中计算 $E - \frac{|\textbf{m}|^2}{2\rho}$  。

  // 类成员  <code>component_names</code>  ,  <code>pressure</code>, and
  // <code>speed_of_sound</code>
  // 的目的从它们的名字中就可以看出。我们还提供了一个函数
  // <code>compute_lambda_max()</code>  ，用于计算上面提到的波速估计，
  // $\lambda_{max}(\mathbf{U},\mathbf{V},\mathbf{n})$  ，用于计算  $d_{ij}$
  // 矩阵。
  ...
  // 最后，我们注意到这是本教程步骤中唯一一个与特定的 "物理学 "或
  // "双曲守恒定律"（本例中为欧拉方程）相关的类。所有其他的类主要是 "离散化
  // "类，与所求解的特定物理学无关。
  ...
  public:
    ...
    static DEAL_II_ALWAYS_INLINE inline double
    pressure(const state_type &U);

    static DEAL_II_ALWAYS_INLINE inline double
    speed_of_sound(const state_type &U);

    static DEAL_II_ALWAYS_INLINE inline flux_type
    flux(const state_type &U);
  ...
  // ，用于计算给定的点和时间的初始状态。这个函数用于填充初始流场，以及在每个时间步长中明确设置迪里切特边界条件（在流入边界）。

  // 在这个例子的步骤中，我们简单地实现了一个均匀的流场，其方向和一维原始状态（密度、速度、压力）从参数文件中读取。

  // 最好是一次性初始化这个类：初始化/设置参数并定义依赖于这些默认参数的类成员。然而，由于我们不知道参数的实际值，这在一般情况下是毫无意义和不安全的（我们希望有机制来检查输入参数的一致性）。我们没有定义另一个
  // <code>setup()</code> 方法在调用 ParameterAcceptor::initialize()
  // 后被调用（手动），而是为类成员 <code>parse_parameters_call_back()</code>
  // 提供了一个 "实现"，当调用 ParameterAcceptor::initialize()
  // 时，每个继承自ParameterAceptor的类都会自动调用。
  ...
  private:
    // 我们声明一个私有的回调函数，它将与
    // ParameterAcceptor::parse_parameters_call_back 信号相连接。
  ...
  // 类在手，我们现在可以实现上面讨论中介绍的显式时间步进方案。
  // <code>%TimeStepping</code> 类的主要方法是<code>make_one_step(vector_type
  // &U, double t)</code>，它接受对状态向量 <code>U</code> and a time point
  // <code>t</code> 的引用（作为输入参数）计算更新的解决方案，将其存储在向量
  // <code>temp</code>, swaps its contents with the vector <code>U</code>
  // 中，并返回选择的 step- 大小 $\tau$  。

  // 另一个重要的方法是  <code>prepare()</code>  ，主要是为临时向量
  ...
  public:
    ...
    double
    make_one_step(vector_type &U, double t);
  ...
  // 在其核心中，Schlieren类实现了类成员  <code>compute_schlieren()</code>
  // 。这个类成员的主要目的是计算一个辅助的有限元场 <code>schlieren</code>
  // ，它在每个节点上由\f[ \text{schlieren}[i] = e^{\beta \frac{ |\nabla r_i| -
  // \min_j |\nabla r_j| }{\max_j |\nabla r_j| - \min_j |\nabla r_j| } },
  // \f]定义，其中 $r$
  // 原则上可以是任何标量。但在实践中，密度是一个自然的候选量，即 $r
  // \dealcoloneq \rho$  。<a
  // href="https:en.wikipedia.org/wiki/Schlieren">Schlieren</a>后处理是一种标准的方法，用于增强可视化的对比度，其灵感来自实际的实验X射线和可视化的阴影技术。(参见
  // step-67  另一个例子，我们创建了一个Schlieren图。)
  ...
  public:
    ...
    void
    compute_schlieren(const vector_type &U);
  ...
  // 现在，剩下的就是把 <code>%TimeStepping</code>, <code>InitialValues</code>
  // , 和  <code>SchlierenPostprocessor</code>
  // 类中实现的方法连在一起。我们在一个单独的类 <code>MainLoop</code>
  // 中做到这一点，该类包含每个类的一个对象，并在ParameterAcceptor类的帮助下再次读入一些参数。
  ...
  private:
    vector_type
    interpolate_initial_values(const double t = 0);

    void
    output(const vector_type &U,
  ...
  // 手头的第一个主要任务是典型的网格生成、数据结构的设置和装配这三者。在这个例子的步骤中，一个值得注意的创新是使用ParameterAcceptor类，我们用它来填充参数值：我们首先初始化ParameterAcceptor类，用一个字符串
  // <code>subsection</code>
  // 表示参数文件中的正确分节，调用它的构造器。然后，在构造函数中，每个参数值都被初始化为一个合理的默认值，并通过调用
  // ParameterAcceptor::add_parameter(). 向ParameterAcceptor类注册。
  template <int dim>
  ...
  // 注意在前面的构造函数中，我们只把MPI通信器传给了 <code>triangulation</code>
  // ，但我们仍然没有初始化底层几何体/网格。如前所述，我们必须将这项任务推迟到
  // <code>setup()</code> 函数，在 ParameterAcceptor::initialize()
  // 函数用从参数文件中读取的最终值填充所有参数变量后，再调用该函数。

  //  <code>setup()</code>
  //  函数是最后一个必须实现的类成员。它创建了实际的三角结构，这是一个基准配置，由一个带有盘状障碍物的通道组成，见
  //  @cite GuermondEtAl2018  。我们通过修改
  //  GridGenerator::hyper_cube_with_cylindrical_hole().
  //  生成的网格来构建几何体。我们参考 step-49 、 step-53 和 step-54
  //  来了解如何创建高级网格。我们首先创建4个临时的（非分布式的）粗略三角形，用
  //  GridGenerator::merge_triangulation() 函数将其缝合起来。我们在 $(0,0)$
  //  处将圆盘居中，直径为 <code>disk_diameter</code>  。通道的左下角有坐标（
  //  <code>-disk_position</code>, <code>-height/2</code>  ），右上角有（
  //  <code>length-disk_position</code>  ,  <code>height/2</code>  ）。
  ...
  Discretization<dim>::setup()
  {
    ...
    // <code>disk_position</code>  . 作为最后一步，边界必须被着色，右边是
    // <code>Boundaries::do_nothing</code> ， <code>dirichlet</code> on the left
    // and <code>free_slip</code> 是上、下外边界和障碍物。
  ...
  // 在 <code>OfflineData</code>
  // 的构造函数中，除了在初始化列表中初始化相应的类成员外，没有做太多的工作。
  ...
  // 现在我们可以初始化DoFHandler，为本地拥有的和本地相关的DOF提取IndexSet对象，并初始化一个
  // Utilities::MPI::Partitioner 对象，这是分布式向量需要的。

  template <int dim>
  void
  OfflineData<dim>::setup()
  {
    ...
    // 不过，这种设计考虑有一个注意事项。让deal.II
    // SparseMatrix类变得快速的是SparsityPattern中使用的<a
    ...
    //  类已经实现了从全局索引范围到连续的局部（每个MPI等级）索引范围的转换：我们不需要重新发明轮子。我们只需要使用这种转换能力（一次，而且只有一次），以便为连续的索引范围创建一个
    //  "本地 "稀疏模式  $[0,$  。
    // <code>n_locally_relevant</code>
    // $)$  . 这种能力可以通过 Utilities::MPI::Partitioner::global_to_local()
    // 函数来调用。一旦使用本地索引创建了稀疏模式，剩下要做的就是确保（在实现我们的scatter和gather辅助函数时）我们总是通过调用
    // LinearAlgebra::distributed::Vector::local_element().
    // 来访问分布式向量的元素，这样我们就完全避免了索引转换，并完全使用本地索引进行操作。

    {
      ...
      // 在本地拥有的DOF的情况下，这种重新编号包括应用一个移位（即我们减去一个偏移量），这样，现在它们将成为整数区间
      // $[0,$ 中的一个数字。 <code>n_locally_owned</code>
      // $)$  .
  ...
  {
    // <code>CopyData</code>
    // 类，将用于使用WorkStream组装离线数据矩阵。它作为一个容器：它只是一个结构，WorkStream在其中存储本地单元的贡献。请注意，它还包含一个类成员
    // <code>local_boundary_normal_map</code>
    // ，用于存储计算边界法线所需的局部贡献。
    ...
    // 接下来我们介绍一些辅助函数，它们都是关于读写矩阵和向量条目的。它们的主要动机是提供稍微有效的代码和<a
    // href="https:en.wikipedia.org/wiki/Syntactic_sugar"> syntactic
    // sugar</a>的代码，否则就有些乏味了。

    // 我们介绍的第一个函数  <code>get_entry()</code>
    // ，将用于读取SparsityPattern迭代器  <code>it</code> of <code>matrix</code>
    // 指向的条目所存储的值。该函数绕过了SparseMatrix接口中的一个小缺陷。SparsityPattern关注的是以CRS格式存储的稀疏矩阵的所有索引操作。因此，迭代器已经知道存储在SparseMatrix对象中的低级向量中相应矩阵条目的全局索引。由于SparseMatrix中缺乏直接用SparsityPattern迭代器访问该元素的接口，不幸的是我们必须创建一个临时的SparseMatrix迭代器。我们只需将其隐藏在
    // <code>get_entry()</code> 函数中。
    ...
    //  <code>set_entry()</code> 帮助器是 <code>get_value()</code>
    //  的逆运算：给定一个迭代器和一个值，它在矩阵中设置迭代器所指向的条目。
    ...
    // 。这基本上意味着我们需要每个空间维度的一个矩阵来存储 $\mathbf{c}_{ij}$
    // 向量。对于矩阵 $\mathbf{n}_{ij}$ 也有类似的观察。
    // <code>gather_get_entry()</code> 的目的是检索这些条目并将其存储到
    // <code>Tensor<1, dim></code> 中，以方便我们使用。
    ...
    // <code>gather()</code>
    // （第一个接口）：这个第一个函数签名，有三个输入参数，将被用来检索矩阵的各个组成部分
    // <code>(i,l)</code> 。 <code>gather_get_entry()</code> 和
    // <code>gather()</code> 的功能非常相同，但它们的背景不同：函数
    // <code>gather()</code>
    // 不依赖迭代器（实际上知道指向的值），而是依赖条目的索引 <code>(i,l)</code>
    // ，以便检索其实际值。我们应该期望  <code>gather()</code>  比
    // <code>gather_get_entry()</code>  稍微昂贵一些。 <code>gather()</code>
    // 的使用将限于计算代数粘度 $d_{ij}$ 的任务，在特殊情况下，当 $i$ 和 $j$
    // 都位于边界时。
    ...
    // <code>gather()</code>
    // （第二个接口）：这个有两个输入参数的第二个函数签名将被用来收集节点
    // <code>i</code> 的状态，并作为 <code>Tensor<1,problem_dimension></code>
    // 返回，以方便我们使用。
    ...
    // <code>scatter()</code>  ：这个函数有三个输入参数，第一个是指一个
    // "全局对象"（比如一个本地拥有的或本地相关的矢量），第二个参数可以是一个
    // <code>Tensor<1,problem_dimension></code>
    // ，最后一个参数代表全局对象的索引。这个函数主要用于将更新的节点值（存储为
    // <code>Tensor<1,problem_dimension></code> ）写入全局对象中。
  ...
  // 我们现在可以将储存在 <code>OfflineData</code>
  // 中的所有矩阵集合起来：质量分录 $m_i$ ，矢量值矩阵 $\mathbf{c}_{ij}$ 和
  // $\mathbf{n}_{ij} = \frac{\mathbf{c}_{ij}}{|\mathbf{c}_{ij}|}$
  // ，以及边界法线 $\boldsymbol{\nu}_i$  。

  // 为了利用线程并行化，我们使用了 @ref threads "多处理器的并行计算 "中详述的WorkStream方法来访问共享内存。按照惯例，这需要定义

  // - 抓取数据（即进行计算所需的输入信息）：在这种情况下，它是
  // <code>scratch_data</code>  。

  // - 工作者：在我们的例子中，这是一个 <code>local_assemble_system()</code>
  // 函数，它实际上是从抓取数据中计算出本地（即当前单元）贡献。

  // - 拷贝数据：一个包含所有本地装配贡献的结构，在这里是
  // <code>CopyData<dim>()</code>  。

  // - 一个拷贝数据的程序：在这种情况下，它是
  // <code>copy_local_to_global()</code>
  // ，负责将这些局部贡献实际复制到全局对象（矩阵和/或矢量）中。

  // 下面的大部分行是用来定义工作者  <code>local_assemble_system()</code>
  // 和复制数据例程  <code>copy_local_to_global()</code>
  // 的。关于WorkStream框架没有太多可说的，因为绝大多数的想法在  step-9  、
  // step-13  和  step-32  等文件中都有合理的记载。
  ...
  // 我们将首先计算这个表达式的分子，并将其存储在
  // <code>OfflineData<dim>::BoundaryNormalMap</code>
  // 中。我们将在一个后置循环中对这些向量进行归一化处理。

  template <int dim>
  void
  OfflineData<dim>::assemble()
  {
...

    {
      ...
      const auto local_assemble_system = //
        [&](const typename DoFHandler<dim>::cell_iterator &cell,
          ...
          // 我们以通常的方式计算凑合质量矩阵项 $m_i$ 和向量 $c_{ij}$
          // 的局部贡献。
              ...
              for (unsigned int j = 0; j < dofs_per_cell; ++j)
                {
                  ...
                  // 注意，"normal
                  // "只代表形状函数phi_j支持下的一个面的贡献。所以我们不能在这里对这个局部贡献进行归一化处理，我们必须
                  // "原封不动
                  // "地接受它，存储它并将它传递给复制数据例程。正确的归一化需要在节点上增加一个循环。这在下面的复制函数中完成。
    ...
    // 此时我们已经完成了 $m_i$ 和 $\mathbf{c}_{ij}$ 的计算，但到目前为止，矩阵
    // <code>nij_matrix</code> 只包含矩阵 <code>cij_matrix</code>
    // 的一个副本。这不是我们真正想要的：我们必须对其条目进行标准化处理。此外，我们还没有填充矩阵
    // <code>norm_matrix</code> 的条目，存储在映射
    // <code>OfflineData<dim>::BoundaryNormalMap</code> 中的向量没有被归一化。

    // 原则上，这只是离线数据，过度优化它们的计算并没有什么意义，因为它们的成本会在我们将要使用的许多时间步骤中得到摊销。然而，计算/存储矩阵
    ...
    // 我们有线程并行化能力 parallel::apply_to_subranges()
    // ，在某种程度上比WorkStream框架更通用。特别是，
    // parallel::apply_to_subranges()
    // 可以用于我们的节点循环。这个功能需要四个输入参数，我们详细解释一下（针对我们的线程并行节点循环的具体案例）。

    // - 迭代器  <code>indices.begin()</code>  指向一个行索引。

    // - 迭代器 <code>indices.end()</code> 指向一个数字上更高的行索引。

    // - 函数 <code>on_subranges(i1,i2)</code> (where <code>i1</code> 和
    // <code>i2</code>
    // 在前面两个子弹中定义的end和begin迭代器所跨越的范围内定义了一个子范围）对这个子范围内的每个迭代器应用一个操作。我们也可以把
    // <code>on_subranges</code> 称为 "工作者"。

    // -
    // Grainsize：每个线程处理的最小迭代器（在本例中代表行）的数量。我们决定最小为4096行。

    // 一个小的注意事项是，提供给 parallel::apply_to_subranges() 的迭代器
    // <code>indices.begin()</code> 和 <code>indices.end()</code>
    // 必须是随机访问的迭代器：在内部， parallel::apply_to_subranges() 将把
    // <code>indices.begin()</code> 和 <code>indices.end()</code>
    // 迭代器定义的范围分成子范围（我们希望能够以恒定的复杂性读取这些子范围的任何条目）。为了提供这样的迭代器，我们求助于
    // std_cxx20::ranges::iota_view. 。

    // 下面这段代码的大部分是用来定义 "工作者" <code>on_subranges</code>
    // ：即在子范围的每一行应用的操作。给定一个固定的 <code>row_index</code>
    // ，我们要访问这一行的每一列/每一个条目。为了执行这样的列-循环，我们使用标准库中的<a
    ...
    // -  <code>sparsity_pattern.end(row_index)</code>
    // 是一个指向该行最后一列的迭代器。

    // -  `std::for_each`
    // 所要求的最后一个参数是应用于该行的每个非零条目（本例中为lambda表达式）的操作。

    // 我们注意到， parallel::apply_to_subranges()
    // 将对不相交的行集（子行）进行操作，我们的目标是写入这些行中。由于我们要进行的操作的简单性质（法线的计算和存储，以及条目
    // $\mathbf{c}_{ij}$
    // 的规范化），线程在试图写同一个条目时不会发生冲突（我们不需要一个调度器）。

    {
      ...
      // 最后，我们对存储在  <code>OfflineData<dim>::BoundaryNormalMap</code>
      // 中的向量进行规范化。这个操作没有被线程并行化，因为它既不能说明任何重要的概念，也不能带来任何明显的速度提升。
  ...
  // 在这一节中，我们描述了 <code>ProblemDescription</code>
  // 类的成员的实现。这里的大部分代码都是针对具有理想气体定律的可压缩欧拉方程的。如果我们想把
  // step-69
  // 重新用于不同的守恒定律（例如：浅水方程），那么这个类的大部分实现就必须改变。但是其他大部分的类（尤其是那些定义循环结构的类）将保持不变。

  // 我们首先实现一些小的成员函数来计算 <code>momentum</code>,
  // <code>internal_energy</code> 、 <code>pressure</code>,
  // <code>speed_of_sound</code> 和系统的通量 <code>f</code>
  // 。这些函数中的每一个的功能都可以从它们的名字中不难看出。
  ...
  // 现在我们讨论  $\lambda_{\text{max}} (\mathbf{U}_i^{n},\mathbf{U}_j^{n},
  // \textbf{n}_{ij})$
  // 的计算。黎曼问题的最大波速的尖锐上界的分析和推导是一个非常技术性的工作，我们不能在本教程中对其进行高级讨论。在这部分文档中，我们将仅限于简述我们实现函数的主要功能，并指出具体的学术参考文献，以帮助（感兴趣的）读者追溯这些想法的来源（和适当的数学证明）。

  // 一般来说，要获得最大波速的尖锐保证上界需要解决一个相当昂贵的标量非线性问题。这通常是通过一个迭代求解器来完成的。为了简化本例中的表述，我们决定不包括这样的迭代方案。相反，我们将只是使用一个初始猜测作为最大波速的上限猜测。更确切地说，
  // @cite GuermondPopov2016b
  // 的方程（2.11）（3.7）、（3.8）和（4.3）足以定义最大波速的保证上限。这个估计值通过调用函数
  // <code>lambda_max_two_rarefaction()</code>
  // 来返回。在其核心部分，这样一个上界的构造使用了所谓的中间压力的二赖式近似
  // $p^*$  ，例如，见公式（4.46），在  @cite Toro2009  第128页。

  // 由 <code>lambda_max_two_rarefaction()</code>
  // 返回的估计值保证是一个上界，它在一般情况下是相当尖锐的，而且对我们的目的来说总体上是足够的。然而，对于一些特定的情况（特别是当其中一个状态接近真空条件时），这样的估计会过于悲观。这就是为什么我们使用第二个估计来避免这种退化，它将通过调用函数
  // <code>lambda_max_expansion()</code>  来调用。这里最重要的函数是
  // <code>compute_lambda_max()</code>  ，它取的是
  // <code>lambda_max_two_rarefaction()</code>  和
  // <code>lambda_max_expansion()</code>  所返回的估计值之间的最小值。

  // 我们再次开始定义几个辅助函数。

  // 第一个函数接收一个状态 <code>U</code> 和一个单位向量 <code>n_ij</code>
  // ，并按照单位向量的方向计算<i>projected</i>一维状态。

  namespace
  {
    template <int dim>
    DEAL_II_ALWAYS_INLINE inline std::array<double, 4>
      ...
      const Tensor<1, dim> &                             n_ij)
    {
      ...
      // 为此，我们必须将动量改为 $\textbf{m}\cdot n_{ij}$
      // ，并且必须从总能量中减去垂直部分的动能。
      ...
      // 我们以<i>primitive</i>变量而不是守恒量来返回一维状态。返回数组包括密度
      // $\rho$  、速度  $u$  、压力  $p$  和局部声速  $a$  。
    ...
    DEAL_II_ALWAYS_INLINE inline double
    positive_part(const double number)
    {
      return std::max(number, 0.);
    }

    DEAL_II_ALWAYS_INLINE inline double
    negative_part(const double number)
    {
      return -std::min(number, 0.);
    }

    // 接下来，我们需要两个本地文数，它们是以原始状态 $[\rho, u, p, a]$
    // 和给定压力 $p^\ast$ 为条件定义的。
    ...
    lambda3_plus(const std::array<double, 4> &riemann_data, const double p_star)
    {
      ...
      中的公式（3 .8）。 constexpr double gamma = ProblemDescription<1>::gamma;
      const auto                          u     = riemann_data[1];
      const auto                          p     = riemann_data[2];
      const auto                          a     = riemann_data[3];
    ...
    // 剩下的就是计算从左和右原始状态计算出来的 $\lambda^-$ 和 $\lambda^+$
    // 的最大值（  @cite GuermondPopov2016  公式（2.11）），其中 $p^\ast$ 由
    // @cite GuermondPopov2016  公式（4.3）给出。
  ...
  // 下面是我们要调用的主函数，以计算  $\lambda_{\text{max}}
  // (\mathbf{U}_i^{n},\mathbf{U}_j^{n}, \textbf{n}_{ij})$
  // 。我们简单地计算两个最大的波速估计值并返回最小值。
  ...
  // 我们通过定义静态数组 <code>component_names</code>
  // 来结束本节，这些静态数组包含描述我们的状态向量的组件名称的字符串。我们对维度一、二和三进行了模板特化，这在后面的DataOut中被用来命名相应的组件。
  ...
  // 在构造函数中，我们用默认值初始化所有参数，为`参数接受器`类声明所有参数，并将
  // <code>parse_parameters_call_back</code> 槽连接到相应的信号。

  //  <code>parse_parameters_call_back</code> 槽将在调用
  //  ParameterAcceptor::initialize(). 后从ParameterAceptor中调用。
  //  在这方面，它的使用适合于参数必须被后处理（在某种意义上）或必须检查参数之间的某些一致性条件的情况。
  ...
  // 到目前为止， <code>InitialValues</code> 的构造函数已经为两个私有成员
  // <code>initial_direction</code> and <code>initial_1d_state</code>
  // 定义了默认值，并将它们添加到参数列表中。但是我们还没有定义我们真正关心的唯一公共成员的实现，也就是
  // <code>initial_state()</code>
  // （我们将调用这个函数来实际评估网格节点的初始解）。在该函数的顶部，我们必须确保提供的初始方向不是零矢量。
  ...
  template <int dim>
  void
  InitialValues<dim>::parse_parameters_callback()
  {
    ...
    // 接下来，我们用一个计算均匀流场的lambda函数来实现
    // <code>initial_state</code>
    // 函数对象。为此，我们必须将给定的原始1d状态（密度 $\rho$ 、速度 $u$ 和压力
    // $p$ ）转换为保守的n维状态（密度 $\rho$ 、动量 $\mathbf{m}$ 和总能量 $E$
    // ）。
  ...
  // 在类成员  <code>prepare()</code>  中我们初始化了临时向量  <code>temp</code>
  // and the matrix <code>dij_matrix</code>  。该向量 <code>temp</code>
  // 将在其内容与旧向量交换之前用于临时存储解决方案的更新。

  template <int dim>
  void
  TimeStepping<dim>::prepare()
  {
    TimerOutput::Scope scope(computing_timer,
  ...
  // 现在是实现正向欧拉步骤的时候了。给出一个在时间 $t$ 的旧状态 <code>U</code>
  // 的（可写引用），我们就地更新状态 <code>U</code>
  // ，并返回所选择的时间步长。我们首先声明一些对各种不同变量和数据结构的只读引用。我们这样做主要是为了有更短的变量名称（例如，
  // <code>sparsity</code> 而不是 <code>offline_data->sparsity_pattern</code>
  // ）。

  template <int dim>
  double
  TimeStepping<dim>::make_one_step(vector_type &U, double t)
  {
    ...
    // 需要强调的是，粘度矩阵必须是对称的，即  $d_{ij} = d_{ji}$
    // 。在这方面我们注意到， $\int_{\Omega} \nabla \phi_j \phi_i \,
    // \mathrm{d}\mathbf{x}= -
    //  \int_{\Omega} \nabla \phi_i \phi_j \, \mathrm{d}\mathbf{x}$ （或等同于
    //  $\mathbf{c}_{ij} = - \mathbf{c}_{ji}$ ）提供了 $\mathbf{x}_i$ 或
    //  $\mathbf{x}_j$
    //  是一个位于远离边界的支持点。在这种情况下，我们可以通过构造检查出
    ...
    // 然而，如果两个支持点 $\mathbf{x}_i$ 或 $\mathbf{x}_j$
    // 恰好都位于边界上，那么，等式 $\mathbf{c}_{ij} =
    ...
    //  就不一定成立。对于这个难题，数学上唯一安全的解决方案是计算 $d_{ij}$ 和
    //  $d_{ji}$ ，并取其最大值。

    // 总体而言， $d_{ij}$
    // 的计算是相当昂贵的。为了节省一些计算时间，我们利用了粘度矩阵必须是对称的这一事实（如上所述）：我们只计算
    // $d_{ij}$ 的上三角条目，并将相应的条目复制到下三角的对应项上。

    // 我们再次使用 parallel::apply_to_subranges() 来实现线程并行的for
    // loops。我们在讨论矩阵的组装 <code>norm_matrix</code> 和上面
    // <code>nij_matrix</code>
    // 的归一化时介绍的几乎所有并行遍历的想法都在这里得到了应用。

    // 我们再次定义了一个 "工作者 "函数 <code>on_subranges</code>
    // ，计算列索引子范围[i1, i2]的黏度 $d_{ij}$ 。
        ...
        [&](const auto i1, const auto i2) {
          for (const auto i :
                                                                        ...
                                                                        *i2))
            {
              ...
              // 对于一个给定的列索引i，我们遍历从
              // <code>sparsity.begin(i)</code> 到 <code>sparsity.end(i)</code>
              // 的稀疏模式的列。
              ...
              for (auto jt = sparsity.begin(i); jt != sparsity.end(i); ++jt)
                {
                  ...
                  // 如果两个支持点刚好都在边界上，我们也要计算 $d_{ji}$
                  // ，然后再取 $\max(d_{ij},d_{ji})$
                  // 。在这之后，我们可以最终设定上三角和下三角的条目。
    ...
    // 到目前为止，我们已经计算了矩阵 <code>dij_matrix</code>
    // 的所有非对角线项。我们仍然需要填补其对角线项，定义为  $d_{ii}^n = -
    // \sum_{j \in \mathcal{I}(i)\backslash \{i\}} d_{ij}^n$  。我们再次使用
    // parallel::apply_to_subranges() 来实现这一目的。在计算 $d_{ii}$
    // s的同时，我们也确定了最大的可接受的时间步长，定义为 \f[
    //    \tau_n \dealcoloneq c_{\text{cfl}}\,\min_{i\in\mathcal{V}}
    //    \left(\frac{m_i}{-2\,d_{ii}^{n}}\right) \, .
    //  \f]
    //  注意， $\min_{i \in \mathcal{V}}$
    //  的操作本质上是全局的，它在所有节点上操作：首先我们必须在所有线程（特定节点的）上取最小值，然后我们必须在所有MPI进程上取最小值。在目前的实现中。
    ...
    // 的内部实现将在一个以上的线程试图同时读取和/或写入 <code>tau_max</code>
    // 时，负责保护任何可能的竞赛条件。

    // - 为了取所有MPI进程的最小值，我们使用实用函数
...

    {
      ...
      // on_subranges()将在每个线程上单独执行。因此，变量
      // <code>tau_max_on_subrange</code> 被存储在线程本地。
      ...
      const auto on_subranges = //
        [&](const auto i1, const auto i2) {
          ...
          // <code>tau_max_on_subrange</code>
          // 包含为（线程局部）子范围计算的最大可能的时间步长。在这一点上，我们必须在所有线程上同步该值。这就是我们使用<a
          //  href="http:www.cplusplus.com/reference/atomic/atomic/"><code>std::atomic<double></code></a>
          //  的原因。
    ...
    // 在这一点上，我们已经计算了所有的粘性系数  $d_{ij}$
    // 并且我们知道最大的可接受的时间步长  $\tau_{\text{max}}$
    // 。这意味着我们现在可以计算更新了。

    // \f[
    \mathbf{U} _i ^ { n + 1 } = \mathbf{U} _i ^ { n } - \frac{\tau_{\text{max}}}
    {
      m_i
    }
 ...
 \f]
   ...
   //<b>Step 5</b>:
   //我们现在在所有MPI行列上更新幽灵层，将临时向量与解决方案向量交换
   //<code>U</code>  （将通过引用返回），并返回选择的时间步长
   //$\tau_{\text{max}}$  。
  ...
  // 在不同的时间间隔内，我们将输出解决方案的当前状态 <code>U</code>
  // 以及所谓的Schlieren图。 <code>SchlierenPostprocessor</code>
  // 类的构造函数同样不包含任何惊喜。我们只是提供默认值并注册两个参数。

  // - schlieren_beta:
  // 是一个临时的正向放大系数，以增强可视化中的对比度。它的实际值是一个品味问题。

  // - schlieren_index: 是一个整数，表示我们将使用状态 $[\rho, \mathbf{m},E]$
  // 中的哪个组件来生成可视化。
  ...
  // 同样， <code>prepare()</code> 函数初始化了两个临时向量（  <code>r</code>
  // and <code>schlieren</code>  ）。

  template <int dim>
  void
  SchlierenPostprocessor<dim>::prepare()
  {
    TimerOutput::Scope scope(computing_timer,
  ...
  // 我们现在讨论类成员
  // <code>SchlierenPostprocessor<dim>::compute_schlieren()</code>
  // 的实现，它基本上是取状态向量 <code>U</code>
  // 的一个分量并计算该分量的Schlieren指标（Schlieren指标的公式可以在类的声明
  // <code>SchlierenPostprocessor</code> 之前找到）。我们首先注意到这个公式需要
  // "结点梯度"  $\nabla r_j$  。然而，对于  $\mathcal{C}^0$
  // 有限元函数来说，梯度的节点值并没有定义。更为普遍的是，梯度的点值对于
  // $W^{1,p}(\Omega)$
  // 函数没有定义。我们可以用最简单的技术来恢复节点的梯度，即加权平均法。
  ...
  // 其中 $S_i$ 是形状函数 $\phi_i$ 的支持，而 $\omega_i(\mathbf{x})$
  // 是权重。权重可以是任何正函数，如 $\omega_i(\mathbf{x}) \equiv 1$
  // （这将使我们恢复通常的均值概念）。但是像往常一样，我们的目标是尽可能多地重复使用离线数据。在这个意义上，最自然的权重选择是
  // $\omega_i = \phi_i$  。将这种权重的选择和扩展 $r_h(\mathbf{x}) = \sum_{j
  // \in \mathcal{V}} r_j \phi_j(\mathbf{x})$ 插入 $\mathbf{(*)}$ 中，我们得到:
  ...
  // $\min_j |\nabla r_j|$  。按照在类成员
  // <code>%TimeStepping\<dim>::%step()</code>
  // 中用于计算时间步长的相同思路，我们将 $\max_j |\nabla r_j|$ 和 $\min_j
  // |\nabla r_j|$
  // 定义为原子双数，以解决线程之间的任何冲突。像往常一样，我们使用
  // <code>Utilities::MPI::max()</code> 和 <code>Utilities::MPI::min()</code>
  // 来寻找所有MPI进程中的全局最大/最小值。
  ...
  template <int dim>
  void
  SchlierenPostprocessor<dim>::compute_schlieren(const vector_type &U)
  {
    TimerOutput::Scope scope(
          ...
          for (const auto i : boost::make_iterator_range(i1, i2))
            {
              ...
              // 我们在自由滑移边界固定梯度r_i，类似于我们在正向欧拉步骤中固定边界状态的方式。
              // 这样可以避免在自由滑移边界的Schlieren图中出现尖锐的、人为的梯度，这纯粹是一种外观上的选择。
              ...
              // 我们提醒读者，我们对结点梯度本身并不感兴趣。我们只想得到它们的规范，以便计算Schlieren指标（用块状质量矩阵
              // $m_i$  加权）。
    ...
    // 第二个循环：我们现在有了矢量 <code>r</code> 和标量 <code>r_i_max</code>
    // and <code>r_i_min</code>
    // 可以使用。这样我们就可以实际计算Schlieren指标了。
  ...
  // 在实现了所有的类之后，是时候创建一个 <code>Discretization<dim></code>,
  // <code>OfflineData<dim></code> 、 <code>InitialValues<dim></code>,
  // <code>%TimeStepping\<dim></code> 和
  // <code>SchlierenPostprocessor<dim></code>
  // 的实例，并在一个循环中运行欧拉正步。

  // 在 <code>MainLoop<dim></code>
  // 的构造函数中，我们现在初始化所有类的实例，并声明一些控制输出的参数。最值得注意的是，我们声明了一个布尔参数
  // <code>resume</code> ，它将控制程序是否试图从中断的计算中重新启动。
  ...
  // 我们首先在匿名命名空间中实现一个辅助函数 <code>print_head()</code>
  // ，用来在终端输出带有一些漂亮格式的信息。

  namespace
  {
    void
    print_head(ConditionalOStream &pcout,
  ...
  // 有了 <code>print_head</code> ，现在是时候实现
  // <code>MainLoop<dim>::run()</code> 了，它包含了我们程序的主循环。

  template <int dim>
  void
  MainLoop<dim>::run()
  {
    // 我们开始读入参数并初始化所有对象。我们在这里注意到，对
    // ParameterAcceptor::initialize
    // 的调用是从参数文件（其名称作为一个字符串参数给出）中读入所有参数。ParameterAcceptor处理一个全局的ParameterHandler，它被初始化为所有从ParameterAceptor派生的类实例的子节和参数声明。调用initialize进入每个每个派生类的分节，并设置所有使用
    // ParameterAcceptor::add_parameter() 添加的变量。
    ...
    // 我们将在变量  <code>t</code> and vector <code>U</code>
    // 中存储当前的时间和状态。
    ...
    // 默认情况下，布尔值 <code>resume</code>
    // 被设置为false，也就是说，下面的代码段不会被运行。然而，如果
    // <code>resume==true</code>
    // ，我们表明我们确实有一个中断的计算，程序应重新启动，从检查点文件中读入由
    // <code>t</code> 、 <code>output_cycle</code>, and <code>U</code>
    // 组成的旧状态。这些检查点文件将在下面讨论的 <code>output()</code>
    // 程序中创建。
    ...
    if (resume)
      {
        ...
        // 我们使用一个 <code>boost::archive</code>
        // 来存储和读入检查点状态的内容。
        ...
        for (auto &it1 : U)
          {
            // <code>it1</code>  遍历状态向量的所有组件  <code>U</code>
            // 。我们依次读入分量的每一个条目，之后更新ghost层。
    ...
    for (unsigned int cycle = 1; t < t_final; ++cycle)
      {
        ...
        // 然后执行一个单一的前向欧拉步骤。请注意，状态向量 <code>U</code>
        // 被就地更新， <code>time_stepping.make_one_step()</code>
        // 返回选择的步长。
        ...
        // -- 特别是在显式时间步进中。因此，我们只在超过
        // <code>output_granularity</code> 设定的阈值时，通过调用
        // <code>output()</code> 函数安排输出。
  ...
  //  <code>interpolate_initial_values</code> 将初始时间 "t "作为输入参数，并在
  //  <code>InitialValues<dim>::initial_state</code> 对象的帮助下填充状态向量
  //  <code>U</code> 。
  ...
  MainLoop<dim>::interpolate_initial_values(const double t)
  {
    ...
    //  <code>InitialValues<dim>::initial_state</code> 的函数签名对于
    //  VectorTools::interpolate().
    //  来说不太合适。我们通过以下方式来解决这个问题：首先，创建一个lambda函数，对于给定的位置
    //  <code>x</code> 只返回 <code>i</code>
    //  的第三部分的值。在ScalarFunctionFromFunctionObject包装器的帮助下，这个lambda又被转换为一个
    //  dealii::Function 。
  ...
  // 写出最终的 vtk 文件是一项相当密集的 IO
  // 任务，会让主循环停滞一段时间。为了避免这种情况，我们使用了<a
  //  href="https:en.wikipedia.org/wiki/Asynchronous_I/O">asynchronous
  //  IO</a>的策略，即创建一个后台线程，在主循环被允许继续的情况下执行IO。为了使其发挥作用，我们必须注意两件事。

  // - 在运行  <code>output_worker</code>  线程之前，我们必须创建一个状态向量
  // <code>U</code>  的副本。我们把它存储在向量  <code>output_vector</code> 中。
  ...
  template <int dim>
  void
  MainLoop<dim>::output(const typename MainLoop<dim>::vector_type &U,
                        const std::string &                        name,
                        const double                               t,
                        ...
                        const bool                                 checkpoint)
  {
    ...
    //  href="https:en.cppreference.com/w/cpp/thread/future"><code>std::future</code></a>对象。这个
    //  <code>std::future</code> 对象包含了函数的返回值，在我们的例子中就是
    //  <code>void</code>  。
    ...
    // 在这一点上，我们制作一份状态向量的副本，运行schlieren后处理器，并运行
    // DataOut<dim>::build_patches()
    // 实际输出代码是标准的。我们创建一个DataOut实例，附加所有我们想要输出的数据向量，并调用
    // DataOut<dim>::build_patches().
    // ，但是有一个转折。为了在后台线程上执行异步IO，我们将DataOut<dim>对象创建为一个共享指针，传递给工作线程，以确保一旦我们退出这个函数，工作线程完成后，DataOut<dim>对象再次被销毁。
    ...
    // <code>this</code>
    // 指针以及输出函数的大部分参数的值，这样我们就可以在lambda函数中访问它们。

    const auto output_worker = [this, name, t, cycle, checkpoint, data_out]() {
      if (checkpoint)
        {
          // 我们通过对<a href="Resume">resume
          // logic</a>的精确反向操作来检查当前状态。
    ...
    //  href="https:en.cppreference.com/w/cpp/thread/future"><code>std::future</code></a>对象，我们可以用它来查询后台线程的状态。在这一点上，我们可以从
    //  <code>output()</code> 函数中返回，继续在主循环中进行时间步进
