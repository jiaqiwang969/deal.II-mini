...
// 最后一步和以前的程序一样。特别是，就像在 step-7
// 中一样，我们把这个程序所特有的一切都打包到一个自己的命名空间中。
...
namespace Step8
{
  ...
  // 唯一的变化是为 <code>fe</code>
  // 变量使用了一个不同的类。我们现在使用的不是FE_Q这样具体的有限元类，而是一个更通用的类，FESystem。事实上，FESystem本身并不是一个真正的有限元，因为它没有实现自己的形状函数。相反，它是一个可以用来将其他几个元素堆叠在一起形成一个矢量值的有限元的类。在我们的例子中，我们将组成
  // <code>FE_Q(1)</code> 对象的矢量值元素，如下所示，在这个类的构造函数中。
                  ...
                  std::vector<Tensor<1, dim>> &  values)
  {
    ...
    for (unsigned int point_n = 0; point_n < points.size(); ++point_n)
      {
        // 如果 <code>points[point_n]</code>
        // 处于围绕这些点之一的半径为0.2的圆（球）中，那么将X方向的力设置为1，否则为0。
        ...
        // 同样地，如果 <code>points[point_n]</code>
        // 在原点附近，那么将y力设置为1，否则为0。
  ...
  // 下面是主类的构造函数。如前所述，我们想构造一个由多个标量有限元组成的矢量值有限元（即，我们想构造矢量值元素，使其每个矢量成分都由一个标量元素的形状函数组成）。当然，我们想堆叠在一起的标量有限元的数量等于解函数的分量数量，由于我们考虑每个空间方向上的位移，所以是
  // <code>dim</code>
  // 。FESystem类可以处理这个问题：我们传递给它我们想组成系统的有限元，以及它的重复频率。
  ...
  // 设置方程组与 step-6
  // 例子中使用的函数相同。DoFHandler类和这里使用的所有其他类都完全知道我们要使用的有限元是矢量值的，并且照顾到了有限元本身的矢量值。(事实上，它们不知道，但这不需要困扰你：因为它们只需要知道每个顶点、直线和单元有多少个自由度，它们不问它们代表什么，也就是说，考虑的有限元是矢量值的，还是例如在每个顶点上有几个自由度的标量Hermite元)。
  ...
  // 这个程序中最大的变化是创建矩阵和右手边，因为它们是取决于问题的。我们将一步一步地完成这个过程
  // step-  ，因为它比以前的例子要复杂一些。

  // 然而，这个函数的前几部分和以前一样：设置一个合适的正交公式，为我们使用的（矢量值）有限元以及正交对象初始化一个FEValues对象，并声明了一些辅助数组。此外，我们还声明了永远相同的两个缩写。
  // <code>n_q_points</code>  和  <code>dofs_per_cell</code>
  // 。每个单元的自由度数量，我们现在显然是从组成的有限元中询问，而不是从底层的标量Q1元中询问。在这里，它是
  // <code>dim</code>
  // 乘以Q1元素的每个单元的自由度数，尽管这不是我们需要关心的明确知识。
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
      {
        ...
        // 然后将局部刚度矩阵的条目和右手边的向量组合起来。这几乎是一对一地遵循本例介绍中描述的模式。
        // 在位的几个评论之一是，我们可以计算数字  <code>comp(i)</code>
        // ，即使用下面的  <code>fe.system_to_component_index(i).first</code>
        // 函数调用形状函数  <code>i</code>  的唯一非零向量成分的索引。

        //（通过访问 <code>system_to_component_index</code> 函数返回值的
        //<code>first</code>
        //变量，你可能已经猜到其中还有更多的内容。事实上，该函数返回一个
        //<code>std::pair@<unsigned int，无符号int @></code>, ，其中第一个元素是
        //<code>comp(i)</code> ，第二个元素是介绍中也指出的值
        //<code>base(i)</code>
        //，即这个形状函数在这个组件中所有非零的形状函数中的索引，即介绍中的字典
        //<code>base(i)</code> 。不过，这不是我们通常感兴趣的数字）。)
                ...
                for (const unsigned int q_point :
                     fe_values.quadrature_point_indices())
                  {
                    cell_matrix(i, j) +=
                      ...
                      // 第一个项是  $\lambda \partial_i u_i, \partial_j v_j) +
                      // (\mu \partial_i u_j, \partial_j v_i)$  。注意，
                      // <code>shape_grad(i,q_point)</code>
                      // 返回正交点q_point处第i个形状函数的唯一非零分量的梯度。梯度的分量
                      // <code>comp(i)</code>
                      // 是第i个形状函数的唯一非零矢量分量相对于comp(i)th坐标的导数，由附加的括号访问。
                      ...
                      (                                                  //
                        ...
                        // 第二个项是  $(\mu \nabla u_i, \nabla v_j)$
                        // 。我们不需要访问梯度的具体分量，因为我们只需要计算两个梯度的标量乘积，这个问题由<tt>operator*</tt>的重载版本来负责，就像前面的例子一样。
  ...
  // 对网格进行细化的函数与 step-6
  // 的例子相同。正交公式再次适应了线性元素。请注意，误差估计器默认情况下是将从有限元解的所有分量中得到的估计值相加，也就是说，它使用所有方向的位移，权重相同。如果我们希望网格只适应x方向的位移，我们可以给函数传递一个额外的参数，告诉它这样做，而不考虑其他所有方向的位移作为误差指标。然而，对于目前的问题，似乎应该考虑所有的位移分量，而且权重相同。
  ...
  // 为了做到这一点， DataOut::add_vector()
  // 函数想要一个字符串的向量。由于分量的数量与我们工作的维数相同，我们使用下面的
  // <code>switch</code> 语句。
  ...
  // 在列出了1d、2d和3d的情况后，如果我们遇到一个我们没有考虑到的情况，让程序死亡是一种很好的风格。请记住，如果第一个参数中的条件没有得到满足，Assert宏会产生一个异常。当然，条件
  // <code>false</code> 永远不可能被满足，所以只要程序运行到默认语句，就会中止。
  ...
  //  <code>run</code> 函数所做的事情与 step-6
  //  中的相同，比如说。这一次，我们使用平方[-1,1]^d作为域，在开始第一次迭代之前，我们在全局上对其进行了四次细化。
  ...
  // 不幸的是，如果离散解是常数，那么KellyErrorEstimator类计算的误差指标对每个单元来说也是零，对
  // Triangulation::refine_and_coarsen_fixed_number()
  // 的调用将不会标记任何单元进行细化（如果每个单元的指示误差是零，为什么要这样做？因此，下一次迭代中的网格也将只由四个单元组成，同样的问题再次发生。
...
// 在上面最后一行关闭了 <code>Step8</code>
// 命名空间后，下面是程序的主要功能，又和 step-6
// 中一模一样（当然，除了改变了类名）。
