...
//导入deal.II的最后一步是这样的。所有deal.II的函数和类都在一个命名空间
//<code>dealii</code>
//中，以确保它们不会与你可能想和deal.II一起使用的其他库的符号发生冲突。我们可以在使用这些函数和类时，在每个名字前加上
//<code>dealii::</code>
//的前缀，但这很快就会变得繁琐和令人厌烦。相反，我们只是简单地导入整个deal.II的名字空间，以供一般使用。
...
first_grid()
{
  ...
  // 接下来，我们要用一个正方形领域的单个单元来填充三角结构。三角形被细化了四次，总共得到
  // $4^4=256$ 个单元。
...
second_grid()
{
  ...
  for (unsigned int step = 0; step < 5; ++step)
    {
      // 接下来，我们需要对三角形的活动单元进行循环。你可以把三角形看作一个单元格的集合。如果它是一个数组，你只需要得到一个指针，用操作符`++`从一个元素递增到下一个元素。三角形的单元不是作为一个简单的数组来存储的，但是<i>iterator</i>的概念将指针的工作方式概括为任意的对象集合（更多信息见<a
      // href=
      // "http:en.wikipedia.org/wiki/Iterator#C.2B.2B">wikipedia</a>）。通常情况下，C++中的任何容器类型都会返回一个迭代器，指向集合的开始，方法称为`begin'，而迭代器则指向集合结束后的1，方法称为`end'。我们可以用操作符`++it`来增加一个迭代器`it`，用`*it`来解除引用以获得底层数据，并通过比较`it
      // != collection.end()`来检查我们是否完成。

      // 第二个重要的部分是我们只需要活动单元。活动单元是那些没有被进一步细化的单元，也是唯一可以被标记为进一步细化的单元。deal.II提供了迭代器类别，允许我们在<i>all</i>单元（包括活动单元的父单元）或只在活动单元上迭代。因为我们要的是后者，所以我们需要调用方法
      ...
      //  @endcode
      //  在一个三角形的所有活动单元上循环。
      //  在这个循环的初始化器中，我们使用了`auto`关键字作为迭代器`it`的类型。`auto`关键字意味着被声明的对象的类型将从上下文中推断出来。当实际的类型名称很长，甚至可能是多余的时候，这个关键字很有用。如果你不确定类型是什么，想查一下结果支持什么操作，你可以去看方法的文档
      //  Triangulation::active_cell_iterators().  在这个例子中，`it`的类型是
      //  `Triangulation::active_cell_iterator`.

      // 虽然`auto`关键字可以让我们不用输入长长的数据类型名称，但我们仍然要输入大量冗余的关于开始和结束迭代器以及如何递增的声明。与其这样，我们不如使用<a
      // href="http:en.cppreference.com/w/cpp/language/range-for">range-based
      // for loops</a>，它将上面显示的所有语法包成一个更短的形式。

      for (auto &cell : triangulation.active_cell_iterators())
        {
          // @note  关于deal.II中使用的迭代器类的更多信息，见 @ref Iterators ，关于基于范围的for循环和`auto`关键字的更多信息，见 @ref CPP11 。

          // 接下来，我们在单元格的所有顶点上循环。为此，我们查询一个顶点索引的迭代器（在2D中，这是一个包含元素`{0,1,2,3}`的数组，但是由于`cell->vertex_indices()`知道单元格所处的维度，因此返回的数组在所有维度上都是正确的，这使得无论我们在2D还是3D中运行这段代码都是正确的，也就是说，它实现了
          // "维度无关的编程" - 我们将在  step-4  中讨论一个重要部分）。

          for (const auto v : cell->vertex_indices())
            {
              // 如果这个单元格位于内边界，那么它至少有一个顶点必须位于内环上，因此与中心的径向距离正好是0.5，达到浮点精度。所以我们计算这个距离，如果我们发现一个顶点具有这个属性，我们就标记这个单元，以便以后进行细化。然后我们也可以打破所有顶点的循环，转到下一个单元。
              // 因为离中心的距离是以浮点数计算的，所以我们必须期望我们所计算的东西只能精确到[round-off](https:en.wikipedia.org/wiki/Round-off_error)以内。因此，我们永远不能指望通过平等的方式来比较距离和内半径。诸如
              // "if (distance_from_center == inner_radius)
              // "这样的语句将会失败，除非我们运气特别好。相反，我们需要以一定的容忍度进行比较，通常的方法是写成`if
              // (std::abs(distance_from_center  。

              // - inner_radius) <=
              // tolerance)`，其中`tolerance'是比四舍五入大的某个小数字。问题是如何选择它。我们可以直接选择，比如说，`1e-10'，但这只适合于我们比较的对象是大小为1的情况。如果我们创建了一个单元大小为`1e+10'的网格，那么`1e-10'将远远低于四舍五入，就像以前一样，只有在我们特别幸运的情况下，比较才会成功。相反，使公差*相对于被比较对象的典型
              // "比例 "几乎总是有用的。在这里，"尺度
              // "是指内半径，或者是细胞的直径。我们选择前者，并将公差设置为
              // $10^{-6}$ 倍环形物的内半径。
