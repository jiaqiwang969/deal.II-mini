...
// 以上是相当常见的包含文件。这些文件还包括稀疏直接类的文件
// SparseDirectUMFPACK。这不是解决大型线性问题的最有效的方法，但现在可以了。
...
// 一个类似的问题出现在系统矩阵和向量中块的排序上。矩阵中有 $9\times 9$
// 块，而且很难记住哪个是哪个。对这些块也使用符号名称要容易得多。
...
namespace SAND
{
  ...
  private:
    ...
    BlockVector<double>
    compute_scaled_step(const BlockVector<double> &state,
  ...
  // 我们初始化一个由2  $\times$  dim
  // `FE_Q(1)`元素组成的FES系统，用于位移变量及其拉格朗日乘数，以及7
  // `FE_DGQ(0)`元素。
  // 这些片状常数函数用于与密度相关的变量：密度本身、未过滤的密度、用于未过滤的密度的下限和上限的松弛变量，然后是用于过滤和未过滤的密度之间的连接以及不等式约束的拉格朗日乘子。
  ...
  // 接下来，确定由于边界值而产生的约束。 域的底角在 $y$
  // 方向保持不变--左下角也在 $x$
  // 方向。deal.II通常认为边界值是附着在边界的片段上的，即面，而不是单个顶点。的确，从数学上讲，对于无穷大的偏微分方程，我们不能把边界值分配给单个点。但是，由于我们试图重现一个广泛使用的基准，我们还是要这样做，并牢记我们有一个有限维的问题，在单个节点上施加边界条件是有效的。
  ...
  // 下一个函数制作了一个巨大的9乘9的块状矩阵，并且还设置了必要的块状向量。
  // 这个矩阵的稀疏度模式包括滤波矩阵的稀疏度模式。它还初始化了我们将使用的任何块向量。
  ...
  SANDTopOpt<dim>::setup_block_system()
  {
    ...
    // "中定义的符号名称有助于理解下面每个项所对应的内容，但它也使表达式变得冗长而不流畅。像
    // `coupling[SolutionComponents::density_upper_slack_multiplier<dim>][SolutionComponents::density<dim>]`
    // 这样的术语读起来就不太顺口，要么必须分成几行，要么几乎跑到每个屏幕的右边缘。因此，我们打开了一个大括号封闭的代码块，在这个代码块中，我们通过说
    // "使用命名空间SolutionComponents"，暂时使命名空间`SolutionComponents'中的名字可用，而不需要命名空间修饰语。
    ...
    // 在生成了 "动态
    // "稀疏度模式之后，我们终于可以将其复制到用于将矩阵与稀疏度模式联系起来的结构中。由于稀疏模式很大很复杂，我们还将其输出到一个自己的文件中，以达到可视化的目的--换句话说，是为了
    // "可视化调试"。
    ...
    // 剩下的就是正确确定各种向量及其块的大小，以及为（非线性）解向量的一些分量设置初始猜测。我们在这里使用解向量各个区块的符号分量名称，为了简洁起见，使用与上面的
    // "使用命名空间 "相同的技巧。
  ...
  // 接下来是一个在程序开始时使用一次的函数。它创建了一个矩阵 $H$
  // ，使过滤后的密度向量等于 $H$ 乘以未过滤的密度。
  // 这个矩阵的创建是非同小可的，它在每次迭代中都会被使用，因此，与其像我们对牛顿矩阵那样对其进行改造，不如只做一次并单独存储。
  ...
  // 这个函数的顶部与大多数此类函数一样，只是设置了实际装配所需的各种变量，包括一大堆提取器。如果你以前看过
  // step-22  ，整个设置应该看起来很熟悉，尽管有些冗长。
  ...
  SANDTopOpt<dim>::assemble_system()
  {
    ...
    // 在这一点上，我们对未过滤的密度进行过滤，并对未过滤的密度乘法器进行邻接（转置）操作，都是对当前非线性解决方案的最佳猜测。后来我们用它来告诉我们，我们过滤的密度与应用于未过滤密度的过滤器有多大的偏差。这是因为在非线性问题的解中，我们有
    // $\rho=H\varrho$ ，但在中间迭代中，我们一般有 $\rho^k\neq H\varrho^k$
    // ，然后 "残差" $\rho^k-H\varrho^k$
    // 将出现在我们下面计算的牛顿更新方程中的右边。
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
      {
        ...
        for (const auto q_point : fe_values.quadrature_point_indices())
          {
            // 我们还需要几个与来自拉格朗日的第一导数的测试函数相对应的数值，也就是
            // $d_{\bullet}$ 函数。这些都是在这里计算的。
            ...
            for (const auto i : fe_values.dof_indices())
              {
                ...
                for (const auto j : fe_values.dof_indices())
                  {
                    // 最后，我们需要来自拉格朗日的第二轮导数的数值，即
                    // $c_{\bullet}$ 函数。这些是在这里计算的。
                    ...
                    // 这就是实际工作的开始。在下文中，我们将建立矩阵的所有项--它们数量众多，而且不完全是不言自明的，也取决于之前的解和它的导数（我们已经在上面评估了这些导数，并将其放入名为`old_*`的变量中）。为了理解这些条款的每一个对应的内容，你要看一下上面介绍中这些条款的明确形式。
                    // 被驱动到0的方程的右边给出了寻找局部最小值的所有KKT条件--每个单独方程的描述都是随着右边的计算给出的。
    ...
    // 这里我们使用我们已经构建好的过滤器矩阵。我们只需要整合这个应用于测试函数的过滤器，它是片状常数，所以整合变成了简单的乘以单元格的度量。
    // 遍历预制的过滤器矩阵可以让我们使用哪些单元格在过滤器中或不在过滤器中的信息，而不需要再次重复检查邻居单元格。
  ...
  // 我们将需要在每次迭代中解决一个线性系统。我们暂时使用一个直接求解器--对于一个有这么多非零值的矩阵来说，这显然不是一个有效的选择，而且它不会扩展到任何有趣的地方。对于
  // "真正的
  // "应用，我们将需要一个迭代求解器，但系统的复杂性意味着一个迭代求解器的算法将需要大量的工作。因为这不是当前程序的重点，所以我们简单地坚持使用我们在这里的直接求解器--该函数遵循与
  // step-29 中使用的相同结构。
  ...
  // 我们先用一个函数进行二进制搜索，找出符合对偶可行性的最大步骤--也就是说，我们能走多远，使
  // $s>0$  和  $z>0$  。该函数返回一对数值，分别代表 $s$ 和 $z$ 的松弛变量。
  ...
  // 下一个函数计算一个围绕 "测试解向量
  // "线性化的右手向量，我们可以用它来观察KKT条件的大小。
  // 然后，这将用于在缩小障碍大小之前测试收敛性，以及计算 $l_1$ 的优点。

  // 这个函数冗长而复杂，但它实际上只是复制了上面`assemble_system()`函数的右侧部分的内容。

  template <int dim>
  BlockVector<double>
  SANDTopOpt<dim>::calculate_test_rhs(
            ...
            for (const auto i : fe_values.dof_indices())
              {
                ...
                /* 方程5：该方程设定了下限的松弛量， giving a minimum density of
                 * 0. */
                ...
                /* 方程6：该方程设定了上层松弛量variable equal to one minus the
                 * unfiltered density. */
  ...
  // 我们在这里使用的算法使用一个 "看门狗 "策略来确定从当前迭代的位置和程度。
  // 我们将看门狗策略建立在一个精确的 $l_1$
  // 功绩函数上。这个函数计算一个给定的、假定的、下一个迭代的精确 $l_1$ 功绩。

  //优点函数由目标函数的总和（简单来说就是外力的积分（在域的边界上）乘以测试解的位移值（通常是当前解加上牛顿更新的某个倍数），以及残差向量的拉格朗日乘数分量的
  //$l_1$ 准则组成。下面的代码依次计算这些部分。

  template <int dim>
  double
  SANDTopOpt<dim>::calculate_exact_merit(
    const BlockVector<double> &test_solution)
  {
    ...
    //然后
    //计算残差，并取对应于拉格朗日多边形的组件的 $l_1$
    //准则。我们把这些加到上面计算的目标函数中，并在底部返回总和。
  ...
  // 这个函数还更新了优点函数中的惩罚乘数，然后返回最大比例的可行步骤。它使用`calculate_max_step_sizes()`函数来找到满足
  // $s>0$  和  $z>0$  的最大可行步骤。
  ...
  SANDTopOpt<dim>::find_max_step()
  {
    ...
    // 接下来我们要更新punice_multiplier。
    // 从本质上讲，更大的惩罚乘数使我们更多考虑约束条件。
    // 观察与我们的决策变量有关的Hessian和梯度，并将其与我们的约束误差的规范相比较，可以确保我们的优点函数是
    // "精确的"

    // 也就是说，它在与目标函数相同的位置有一个最小值。
    // 由于我们的优点函数对任何超过某个最小值的惩罚乘数都是精确的，所以我们只保留计算值，如果它增加了惩罚乘数。
  ...
  // 本块中的最后一个辅助函数是检查是否充分满足KKT条件，以便整个算法可以降低障碍物的大小。它通过计算残差的
  // $l_1$ 准则来实现，这就是`calculate_test_rhs()`的计算。
  ...
  // 后处理函数中的第一个函数在VTU文件中输出信息，用于可视化。它看起来很长，但实际上与
  // step-22  中所做的一样，例如，只是增加了（很多）解决方案的变量。

  template <int dim>
  void
  SANDTopOpt<dim>::output_results(const unsigned int iteration) const
  {
  ...
  // 其中第二个函数将解决方案输出为`.stl`文件，用于3D打印。STL](https:en.wikipedia.org/wiki/STL_(file_format))文件是由三角形和法线向量组成的，我们将用它来显示所有那些密度值大于0的单元，首先将网格从
  // $z$ 值挤出到 $z=0.25$
  // ，然后为密度值足够大的单元的每个面生成两个三角形。当从外面看时，三角形节点必须逆时针走，法向量必须是指向外部的单位向量，这需要进行一些检查。
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
      {
        if (nonlinear_solution.block(
              SolutionBlocks::density)[cell->active_cell_index()] > 0.5)
          {
            ...
            // 接下来我们需要处理单元格的四个面，扩展到 $z$
            // 方向。然而，我们只需要写这些面，如果该面在域的边界上，或者它是密度大于0.5的单元和密度小于0.5的单元之间的界面。
  ...
  // 这个函数最终提供了整体的驱动逻辑。从总体上看，这是一个相当复杂的函数，主要是因为优化算法很困难：它不仅仅是像
  // step-15
  // 中那样找到一个牛顿方向，然后在这个方向上再走一个固定的距离，而是要（i）确定当前步骤中的最佳对数障碍惩罚参数应该是什么，（ii）通过复杂的算法来确定我们要走多远，还有其他成分。让我们看看如何在下面的文件中把它分解成小块。
  ...
  SANDTopOpt<dim>::run()
  {
    ...
    // 现在开始进行主迭代。整个算法通过使用一个外循环来工作，在这个外循环中，我们一直循环到（i）对数障碍参数变得足够小，或者（ii）我们已经达到收敛。在任何情况下，如果最终的迭代次数过多，我们就会终止。这个整体结构被编码为一个
    // "do{ ... } while (...)`循环，其中收敛条件在底部。
    ...
    do
      {
        ...
        // 看门狗算法本身的总体思路是这样的。对于最大的`max_uphill_steps`（即上述
        // "内循环
        // "中的一个循环）的尝试，我们使用`find_max_step()`来计算牛顿更新步骤，并在`nonlinear_solution`向量中加上这些。
        // 在每一次尝试中（从上一次尝试结束时到达的地方开始），我们检查我们是否已经达到了上述优点函数的目标值。目标值是根据本算法的起始位置（看门狗循环开始时的`nonlinear_solution'，保存为`看门狗_state'）和本循环第一个回合中`find_max_step()'提供的第一个建议方向（`k=0'情况）计算的。
        ...
        do
          {
            ...
            //然后
            //算法的下一部分取决于上面的看门狗循环是否成功。如果成功了，那么我们就满意了，不需要进一步的行动。我们只是停留在原地。然而，如果我们在上面的循环中采取了最大数量的不成功的步骤，那么我们就需要做一些别的事情，这就是下面的代码块所做的。
            //具体来说，从上述循环的最后（不成功的）状态开始，我们再寻找一个更新方向，并采取所谓的
            //"伸展步骤"。如果该拉伸状态满足涉及优点函数的条件，那么我们就去那里。另一方面，如果拉伸状态也是不可接受的（就像上面所有的看门狗步骤一样），那么我们就放弃上面所有的看门狗步骤，在我们开始看门狗迭代的地方重新开始--那个地方被存储在上面的`看门狗_状态`变量中。更具体地说，下面的条件首先测试我们是否从`看门狗_state`方向的`first_step`走了一步，或者我们是否可以从拉伸状态再做一次更新来找到一个新的地方。有可能这两种情况实际上都不比我们在看门狗算法开始时的状态好，但即使是这样，那个地方显然是个困难的地方，离开后从另一个地方开始下一次迭代可能是一个有用的策略，最终收敛。
            ...
            if (watchdog_step_found == false)
              {
                ...
                // 如果我们没有得到一个成功的看门狗步骤，我们现在需要决定是回到我们开始的地方，还是使用最终状态。
                // 我们比较这两个位置的优劣，然后从哪个位置取一个按比例的步长。
                // 由于按比例的步长可以保证降低优点，所以我们最终会保留这两个位置中的一个。
        ...
        // 在外循环结束时，我们必须更新屏障参数，为此我们使用以下公式。该函数的其余部分只是检查外循环的收敛条件，如果我们决定终止计算，就把最终的
        // "设计 "写成STL文件，用于3D打印，并输出一些时间信息。
