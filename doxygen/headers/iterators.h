 ...
 * ++</tt>增加到最近的元素，使用<tt>operator -- </tt>减少到之前的元素。
 * 
 * 我们也可以用加法运算符<tt>it=it+n</tt>来跳转<tt>n</tt>元素，并相应地将一些元素向后移动。此外，按照标准模板库的传统，网格提供了成员函数<tt>begin()</tt>和<tt>end()</tt>，分别提供一个集合的第一个元素和一个超前的迭代器。由于有许多不同的迭代器可用，实际上有一整个系列的这类函数，比如<tt>begin_active()</tt>, <tt>begin_face()</tt>，等等。
 ...
 * 等。这些函数依次从存储这些数据的各种数据结构中找出相关的数据。这实际上是如何做到的，以及使用什么数据结构，并不是deal.II中应用的作者真正关心的。特别是，通过隐藏实际的数据结构，我们能够以一种有效的方式存储数据，而不一定是以一种使应用程序编写者容易访问或理解的方式。
 *
 *
